...
fn add_example() {
    ...
    let res = WasmInterpreter::new(wasm)
        .execute("add", vec!["17", "65"], &mut store)
        .unwrap();
...
fn aes_example() {
    ...
    // Create key and data
    let res = interpreter
        .execute("new_array", vec!["16"], &mut store)
        .unwrap();
    ...
    let res = interpreter
        .execute(
            "encrypt_128",
            vec![&data_ptr.to_string(), &key_ptr.to_string()],
            &mut store,
        )
        .unwrap();
    ...
    let res = interpreter
        .execute(
            "decrypt_128",
            vec![&output_ptr.to_string(), &key_ptr.to_string()],
            &mut store,
        )
        .unwrap();
    ...
    interpreter
        .execute("drop_array", vec![&input_ptr.to_string()], &mut store)
        .unwrap();
    interpreter
        .execute("drop_array", vec![&output_ptr.to_string()], &mut store)
        .unwrap();
    interpreter
        .execute("drop_array", vec![&data_ptr.to_string()], &mut store)
        .unwrap();
    interpreter
        .execute("drop_array", vec![&key_ptr.to_string()], &mut store)
        .unwrap();
...
#[test]
fn simd_example() {
    let example = include_bytes!("simd.wasm");
    ...
    let res = program
        .execute("calc_using_simd", vec!["0", "0", "0"], &mut store)
        .unwrap();
    assert_eq!(res, "4");

    let res = program
        .execute("calc_using_simd", vec!["0", "0", "1"], &mut store)
        .unwrap();
    assert_eq!(res, "58");

    let res = program
        .execute("calc_using_simd", vec!["37", "91", "10"], &mut store)
        .unwrap();
    assert_eq!(res, "831745459");
}
